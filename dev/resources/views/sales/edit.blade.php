...
<head>
	...
	<script src="{{ asset('/js/Sales/edit.js')  . $ver_str}} }}" defer></script>
	...
	<link href="{{ asset('/css/Sales/edit.css')  . $ver_str}}" rel="stylesheet">
	
	<title>売上管理・編集フォーム</title>
...
<nav aria-label="breadcrumb">
  <ol class="breadcrumb">
	...
	<li class="breadcrumb-item active" aria-current="page">売上管理・編集フォーム</li>
  </ol>
</nav>
...
<div>
	<div class="form_w" >
		<form method="POST" action="{{ url('sales/update') }}" onsubmit="return checkDoublePress()">
			@csrf
				...
				<div class="col-12 col-md-7">
					<select name="client_id" class="form-control form-control-lg">
						@foreach ($clientList as $client_id => $client_name)
							<option value="{{ $client_id }}" @selected(old('client_id', $ent->client_id) == $client_id)>
								{{ $client_name }}
							</option>
						@endforeach
					</select>
			...
			<div class="row">
				...
				<div class="col-12 col-md-7">
					<input name="sales_amt" type="text"  class="form-control form-control-lg" placeholder="売上額" value="{{old('sales_amt', $ent->sales_amt)}}">
				</div>
			</div>
				...
				<div class="col-12 col-md-7">
					<select name="status" class="form-control form-control-lg">
						@foreach ($salesStatusList as $status => $status_name)
							<option value="{{ $status }}" @selected(old('status', $ent->status) == $status)>
								{{ $status_name }}
							</option>
						@endforeach
					</select>
			...
			<div class="row">
				...
				<div class="col-12 col-md-7">
					<input name="billing_date" type="date"  class="form-control form-control-lg" placeholder="請求日" value="{{old('billing_date', $ent->billing_date)}}" >
				</div>
			</div>
			...
			<div class="row">
				...
				<div class="col-12 col-md-7">
					<input name="billing_amt" type="text"  class="form-control form-control-lg" placeholder="請求額" value="{{old('billing_amt', $ent->billing_amt)}}">
				</div>
			</div>
			...
			<div class="row">
				...
				<div class="col-12 col-md-7">
					<input name="payment_date" type="date"  class="form-control form-control-lg" placeholder="入金日" value="{{old('payment_date', $ent->payment_date)}}" >
				</div>
			</div>
			...
			<div class="row">
				...
				<div class="col-12 col-md-7">
					<input name="payment_amt" type="text"  class="form-control form-control-lg" placeholder="入金額" value="{{old('payment_amt', $ent->payment_amt)}}">
				</div>
			</div>
			...
			<div class="row">
				...
				<div class="col-12 col-md-7">
					<input name="commission" type="text"  class="form-control form-control-lg" placeholder="手数料" value="{{old('commission', $ent->commission)}}">
				</div>
			</div>
			...
			<div class="row">
				...
				<div class="col-12 col-md-7">
					<input name="tax" type="text"  class="form-control form-control-lg" placeholder="消費税" value="{{old('tax', $ent->tax)}}">
				</div>
			</div>
