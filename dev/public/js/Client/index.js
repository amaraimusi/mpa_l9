...
$(()=>{
	...
	// 行入替機能の初期化
	rowExchange = new RowExchange('client_mng_tbl', data, null, (param)=>{
		// 行入替直後のコールバック
		
		// 行入替後、再び行入替しなければ3秒後に自動DB保存が実行される。
		let auto_save_url = 'client/auto_save';
...
function initClmShowHide(){
	...
	csh.init('client_mng_tbl', 'csh_div', iniClmData);
...
function disabledBtn(btnElm, action_flg){
	...
	let url = 'client/disabled'; // Ajax通信先URL
...
function destroyBtn(btnElm){
	...
	let json_str = JSON.stringify(data);//データをJSON文字列にする。
	let url = 'client/destroy'; // Ajax通信先URL
